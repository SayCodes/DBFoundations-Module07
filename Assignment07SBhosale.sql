...
-- 2024-11-25,Sayali Bhosale,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_SayaliBhosale')
	 Begin 
	  Alter Database [Assignment07DB_SayaliBhosale] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_SayaliBhosale;
	 End
	Create Database Assignment07DB_SayaliBhosale;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_SayaliBhosale;
...
FROM vProductInventories AS P 
	LEFT JOIN vProductInventories AS PM
	ON P.ProductName = PM.ProductName
	AND FORMAT (DATEADD(MONTH, -1, Convert(DATE, P.InventoryDate)),'MMMM, yyyy') = PM.InventoryDate
Order By P.ProductName, Convert(DATE, P.InventoryDate);
GO
